...
export default function App() {
  ...
  const goBack = () => {
    // Battle screens - direct to home to prevent loops
    if (["battle","battle_result","room"].includes(screen)) {
      setScreen("home"); setHistory(["home"]); return;
    }
    if (history.length > 1) {
      const newH = [...history]; newH.pop();
      setHistory(newH); setScreen(newH[newH.length-1]);
    } else { setScreen("home"); setHistory(["home"]); }
  };
  ...
  const forgotPassword = async () => {
    if (!em.trim()) { setAuthErr("Email ഇടൂ — Reset link അയക്കാം!"); return; }
    setAuthLoading(true); setAuthErr("");
    try {
      await sendPasswordResetEmail(auth, em.trim());
      setResetSent(true);
    } catch(e) {
      setAuthErr(e.code==="auth/user-not-found"?"❌ Email registered ആയിട്ടില്ല!":"❌ "+e.message);
    }
    setAuthLoading(false);
  };
  ...
  // Battle Timer - only runs when question changes
  useEffect(() => {
    if (screen!=="battle"||!battleStarted||battleQ.length===0) return;
    if (battlePicked!==null) return; // already answered
    setBattleTimer(20);
    clearInterval(battleTimerRef.current);
    battleTimerRef.current = setInterval(()=>{
      setBattleTimer(t=>{
        if(t<=1){ clearInterval(battleTimerRef.current); handleBattleAns(-1); return 0; }
        return t-1;
      });
    },1000);
  ...
  useEffect(() => {
    if (!roomCode) return;
    battleStartedRef.current = false; // reset on room change
    const roomRef = ref(db, `rooms/${roomCode}`);
    const unsub = onValue(roomRef, (snap) => {
      if (!snap.exists()) return;
      const room = snap.val();
      // Only trigger once - prevent loop
      if (room.status === "playing" && !battleStartedRef.current) {
        ...
        setScreen("battle");
        setHistory(["home","battle_select","battle_lobby","room","battle"]);
      }
      // Update room data for live scores
      setRoomData(room);
    });
    return () => unsub();
  }, [roomCode]);
    ...
    <div style={{...S,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
      ...
      <div style={{width:"100%",maxWidth:420}}>
        ...
        <div style={{...glass(),padding:22}}>
          ...
          {resetSent&&<div style={{background:"rgba(16,185,129,0.1)",border:"1px solid rgba(16,185,129,0.3)",borderRadius:10,padding:"10px 12px",color:"#10b981",fontSize:13,marginBottom:10}}>✅ Password reset link sent! Email check ചെയ്യൂ.</div>}
          <button onClick={loginEmail} disabled={authLoading} style={{...Btn("linear-gradient(135deg,#6366f1,#8b5cf6)"),width:"100%",padding:14,fontSize:14,borderRadius:12,marginBottom:10}}>{authLoading?"⏳ Please wait...":authMode==="login"?"🔐 Sign In":"✅ Create Account"}</button>
          {authMode==="login"&&<button onClick={forgotPassword} disabled={authLoading} style={{width:"100%",background:"none",border:"none",color:"#6366f1",fontSize:13,cursor:"pointer",fontFamily:"inherit",fontWeight:600,padding:"4px 0"}}>🔑 Forgot Password?</button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={S}>
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"0 14px 90px"}}>
        ...
        {screen==="forum"&&(
          <div style={{paddingTop:0,display:"flex",flexDirection:"column",height:"calc(100vh - 130px)"}}>
            {/* Forum Header */}
            <div style={{padding:"12px 14px 8px",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
              ...
              <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4}}>
                {[{id:"all",label:"All",icon:"🌐",color:"#6366f1"},...FORUM_CATS].map(cat=>(
                  <button key={cat.id} onClick={()=>setForumCategory(cat.id)} style={{flexShrink:0,padding:"4px 10px",background:forumCategory===cat.id?cat.color:"rgba(255,255,255,0.06)",border:"none",borderRadius:20,color:forumCategory===cat.id?"#fff":"#64748b",cursor:"pointer",fontWeight:700,fontSize:10,whiteSpace:"nowrap",transition:"all 0.2s"}}>
                    {cat.icon} {cat.label}
                  </button>
                ))}
              </div>
            ...
            {/* Messages - WhatsApp style scroll area */}
            <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:6}}>
              {!forumLoaded&&<div style={{textAlign:"center",padding:20,color:"#475569"}}>⏳ Loading...</div>}
              {forumLoaded&&[...forumPosts].filter(p=>forumCategory==="all"||p.category===forumCategory).reverse().map(post=>{
                ...
                return (
                  <div key={post.id} style={{display:"flex",flexDirection:"column",alignItems:isOwn?"flex-end":"flex-start"}}>
                    {/* Name + category (for others only) */}
                    {!isOwn&&<div style={{fontSize:10,color:catInfo.color,fontWeight:700,marginBottom:2,marginLeft:8}}>{post.name} • {catInfo.icon} {catInfo.label}</div>}
                    <div style={{display:"flex",alignItems:"flex-end",gap:6,maxWidth:"80%",flexDirection:isOwn?"row-reverse":"row"}}>
                      {/* Avatar */}
                      {!isOwn&&<div style={{width:28,height:28,borderRadius:"50%",background:`linear-gradient(135deg,${catInfo.color},${catInfo.color}99)`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:12,flexShrink:0}}>
                        {post.avatar||post.name[0].toUpperCase()}
                      </div>}
                      {/* Bubble */}
                      <div style={{background:isOwn?"linear-gradient(135deg,#6366f1,#7c3aed)":"rgba(255,255,255,0.08)",borderRadius:isOwn?"18px 18px 4px 18px":"18px 18px 18px 4px",padding:"10px 14px",position:"relative"}}>
                        <p style={{fontSize:13,color:"#f1f5f9",lineHeight:1.5,margin:0,wordBreak:"break-word"}}>{post.msg}</p>
                    ...
                    {/* Actions */}
                    <div style={{display:"flex",alignItems:"center",gap:8,marginTop:3,paddingLeft:isOwn?0:36,paddingRight:isOwn?0:0}}>
                      <button onClick={()=>likePost(post)} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:11,fontFamily:"inherit",padding:0}}>
                        👍 {post.likes||0}
                      </button>
              ...
              {forumLoaded&&forumPosts.filter(p=>forumCategory==="all"||p.category===forumCategory).length===0&&(
            ...
            {/* Input Box - Fixed at bottom like WhatsApp */}
            <div style={{padding:"8px 10px",background:"rgba(5,5,15,0.95)",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                <select value={forumCategory==="all"?"general":forumCategory} onChange={e=>setForumCategory(e.target.value)} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"5px 10px",color:"#94a3b8",fontSize:11,fontFamily:"inherit",outline:"none"}}>
                  {FORUM_CATS.map(c=><option key={c.id} value={c.id}>{c.icon} {c.label}</option>)}
                </select>
              </div>
              <div style={{display:"flex",alignItems:"flex-end",gap:8}}>
                <div style={{flex:1,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:24,padding:"10px 16px",display:"flex",alignItems:"center"}}>
                  <textarea
                    value={forumMsg}
                    onChange={e=>setForumMsg(e.target.value)}
                    onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); sendForumPost(); } }}
                    placeholder="Type a message..."
                    rows={1}
                    style={{flex:1,background:"none",border:"none",color:"#e2e8f0",fontSize:14,fontFamily:"inherit",resize:"none",outline:"none",maxHeight:80,lineHeight:1.4}}
                  />
                </div>
