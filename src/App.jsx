...
const timeAgo = (ts) => { const d = Date.now() - ts; if (d < 60000) return "now"; if (d < 3600000) return Math.floor(d/60000)+"m ago"; if (d < 86400000) return Math.floor(d/3600000)+"h ago"; return Math.floor(d/86400000)+"d ago"; };
...
function PuterQuizGenerator({ db, categories, user, showNotif }) {
  ...
  const handlePuterLogin = async () => {
    try { await window.puter.auth.signIn(); setIsPuterSignedIn(true); setGenMsg("✅ ലോഗിൻ വിജയകരം!"); showNotif("Puter Login Success!"); } 
  ...
  const generateQuiz = async () => {
    ...
    try {
      ...
      const normalized = parsed.map((q, i) => {
        ...
        else if (typeof q.answer === "string") { const up = q.answer.toUpperCase(); if (up==="A") ansIdx=0; else if (up==="B") ansIdx=1; else if (up==="C") ansIdx=2; else if (up==="D") ansIdx=3; else ansIdx = parseInt(q.answer)||0; }
        return {
          q: q.q || `Question ${i + 1}`, qm: q.qm || "", options: q.options || ["A", "B", "C", "D"], answer: (ansIdx>=0&&ansIdx<=3)?ansIdx:0, explanation: q.explanation || "", cat: targetCat, _selected: true,
        };
      });
  ...
  const Btn = (bg, col="#fff") => ({ background:bg, color:col, border:"none", borderRadius:12, padding:"12px 16px", cursor:"pointer", fontWeight:700, fontSize:13, fontFamily:"inherit", transition:"all 0.2s" });

  return (
    <div style={{background:"rgba(255,255,255,0.03)", borderRadius:16, padding:14, border:"1px solid rgba(255,255,255,0.1)"}}>
      <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:14}}>
        <span style={{fontSize:24}}>🤖</span><div style={{fontWeight:800, color:"#6ee7b7"}}>AI Quiz Generator</div>
      </div>
      <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Topic (e.g. Kerala History)" style={Inp} />
      <div style={{display:"flex", gap:8, marginBottom:10}}>
        <select value={targetCat} onChange={e => setTargetCat(e.target.value)} style={{...Inp, flex:1, marginBottom:0}}>
          {categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.label}</option>)}
        </select>
        <select value={qCount} onChange={e => setQCount(Number(e.target.value))} style={{...Inp, flex:1, marginBottom:0}}>
      ...
      {!isPuterSignedIn ? (
        <button onClick={handlePuterLogin} style={{...Btn("linear-gradient(135deg,#f59e0b,#d97706)"), width:"100%"}}>🔐 Login to Puter AI</button>
      ) : (
      ...
      {generatedQs.length > 0 && (
        <div style={{marginTop:15}}>
          {generatedQs.map((q, idx) => (
            <div key={idx} style={{background:"rgba(255,255,255,0.05)", padding:10, borderRadius:8, marginBottom:8}}>
              <div style={{display:"flex", gap:8}}>
                <input type="checkbox" checked={q._selected} onChange={() => setGeneratedQs(prev => prev.map((item, i) => i === idx ? { ...item, _selected: !item._selected } : item))} style={{marginTop:4}}/>
                <div>
                  <div style={{fontSize:12, fontWeight:"bold", color:"#fff"}}>{q.q}</div>
                  <div style={{fontSize:10, color:"#10b981", marginTop:4}}>Ans: {q.options[q.answer]}</div>
                </div>
              </div>
            </div>
          ))}
...
export default function App() {
  ...
  const [sheetId, setSheetId] = useState(""); const [sheetCat, setSheetCat] = useState("ldc");
  ...
  const loadData = (uid, admin) => {
    ...
    onValue(query(ref(db,"leaderboard"), orderByChild("score"), limitToLast(20)), snap => { if(snap.exists()) { const d=[]; snap.forEach(c=>d.push({id:c.key,...c.val()})); setLeaderboard(d.reverse()); } });
  ...
  useEffect(() => {
    ...
    const chatUnsub = onValue(ref(db,`rooms/${roomCode}/chat`), snap => { if(snap.exists()) { const d=[]; snap.forEach(c=>d.push({id:c.key,...c.val()})); setChatMsgs(d); } });
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"15px"}}>
        ...
        {screen==="battle_select"&&(
          <div>
            ...
            {[{type:"1v1",i:"🤺",t:"1 vs 1 Battle",s:"Play with a friend"},{type:"multi",i:"👥",t:"Multiplayer",s:"Play with many"},{type:"random",i:"🎲",t:"Random Match",s:"Play with AI if no users"}].map(b=>(
        ...
        {screen==="admin"&&isAdmin&&(
          <div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
              {[["pending",`⏳ Pend (${pendingQ.length})`],["ai","🤖 AI Gen"],["bulk","📋 Bulk"],["addq","➕ Add"],["cats","📁 Cats"],["sheet","📊 Sheet"],["admins","👑 Admins"],["members","🟢 Users"],["fix","🛠️ Fix DB"]].map(([t,l])=>(
            ...
            {adminTab==="addq"&&isSuperAdmin&&(
              <div style={{...card(),padding:20}}>
                ...
                <select value={newQ.answer} onChange={e=>setNewQ({...newQ,answer:e.target.value})} style={Inp}><option value="0">Ans: 1</option><option value="1">Ans: 2</option><option value="2">Ans: 3</option><option value="3">Ans: 4</option></select>
            ...
            {adminTab==="sheet"&&isSuperAdmin&&(
              <div style={{...card(),padding:20}}>
                <h3 style={{color:"#3b82f6",marginBottom:10}}>Import Public Google Sheet</h3>
                <input value={sheetId} onChange={e=>setSheetId(e.target.value)} placeholder="Sheet ID" style={Inp}/>
                <p style={{fontSize:10,color:"#888"}}>Ensure sheet is "Anyone with link can view". Same column format as bulk upload.</p>
              </div>
            )}
