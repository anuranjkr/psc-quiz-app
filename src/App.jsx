...
function PuterQuizGenerator({ db, categories, user, showNotif }) {
  ...
  const generateQuiz = async () => {
    ...
    try {
      const malayalamInstruction = includeMalayalam ? `Also provide a Malayalam translation in "qm" field.` : `Leave "qm" as "".`;
      const prompt = `Generate exactly ${qCount} MCQs about "${topic}". Category: ${targetCat}. Return ONLY a JSON array: [{"q":"?", "qm":"", "options":["","","",""], "answer":0, "explanation":""}]`;
      ...
      const normalized = parsed.map(q => ({
        ...
        cat: targetCat.toLowerCase().trim(), // Force Lowercase & Trim
        _selected: true
      }));
  ...
  return (
    <div style={{background:"rgba(255,255,255,0.03)", padding:12, borderRadius:12, marginBottom:15}}>
      <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Enter Topic (e.g. India Rivers)" style={{width:"100%", padding:10, borderRadius:8, background:"#111", color:"#fff", border:"1px solid #333", marginBottom:8}} />
      <select value={targetCat} onChange={e => setTargetCat(e.target.value)} style={{width:"100%", padding:10, borderRadius:8, background:"#111", color:"#fff", border:"1px solid #333", marginBottom:8}}>
        {categories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
      </select>
      <p style={{fontSize:12, color:"#aaa"}}>{genMsg}</p>
      {!isPuterSignedIn ? <button onClick={handlePuterLogin} style={{padding:10, background:"#f59e0b", border:"none", borderRadius:8, width:"100%", fontWeight:"bold", cursor:"pointer"}}>Login to Puter AI</button> : <button onClick={generateQuiz} disabled={genStatus==="loading"} style={{padding:10, background:"#10b981", border:"none", borderRadius:8, width:"100%", fontWeight:"bold", color:"#fff", cursor:"pointer"}}>🤖 Generate Questions</button>}
      {generatedQs.length > 0 && <button onClick={uploadSelected} style={{padding:10, background:"#6366f1", border:"none", borderRadius:8, width:"100%", fontWeight:"bold", color:"#fff", marginTop:8, cursor:"pointer"}}>🚀 Upload {generatedQs.filter(q=>q._selected).length} Qs to Firebase</button>}
    </div>
  );
}
...
export default function App() {
  ...
  const [dbError, setDbError] = useState(""); // 🔴 ഫയർബേസ് എറർ കാണിക്കാൻ പുതിയ സ്റ്റേറ്റ്
  ...
  // 🔴 കാറ്റഗറി മാച്ചിങ് കൂടുതൽ സ്ട്രോങ്ങ് ആക്കി (Case-insensitive & Trimed)
  useEffect(() => {
  ...
  const loadData = (uid) => {
    setDbError("");
    // 🔴 റീഡ് എറർ ഉണ്ടെങ്കിൽ അത് ലൈവായി പിടിച്ചെടുക്കുന്നു
    onValue(ref(db, "questions"), snap => {
        ...
        snap.forEach(c => {
          ...
          if (val) {
            qs.push({
              id: c.key,
              ...val,
              cat: val.cat ? String(val.cat).toLowerCase().trim() : "ldc" // തനിയെ ഫോർമാറ്റ് തിരുത്തുന്നു
            });
          }
        });
      }
      ...
      setDbError(err.message); // Rules expired ആയാൽ ഇവിടെ മെസ്സേജ് വരും
    });
      ...
      <div style={{maxWidth:500, margin:"0 auto", padding:15}}>
        ...
        {dbError && (
          <div style={{background:"rgba(239,68,68,0.2)", border:"1px solid #ef4444", padding:12, borderRadius:10, marginBottom:15, color:"#fca5a5"}}>
            ...
            <p style={{fontSize:12, margin:"4px 0 0"}}>{dbError}. (നിങ്ങളുടെ ഫയർബേസ് ഡാറ്റാബേസ് **Rules** കാലഹരണപ്പെട്ടതാകാം. Firebase Console -&gt; Realtime Database -&gt; Rules എന്നതിൽ പോയി read, write എന്നിവ `true` ആക്കുക).</p>
          </div>
        )}
          ...
          <div>
            ...
            {adminTab === "fix" && (
              <div style={{background:"#0f0f25", padding:15, borderRadius:12, border:"1px solid #ef4444"}}>
                ...
                {/* 🔴 ഉപയോക്താവിന് ഡാറ്റാബേസ് സ്ട്രക്ചർ നേരിട്ട് കണ്ട് തിരുത്താനുള്ള ടൂൾ */}
